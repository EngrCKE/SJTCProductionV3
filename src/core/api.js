...
function demoApi(action, body){
  ...
  if(action === "getProductionLogs") {
    const logs = demo.logs.map(l=>{
      const item=demo.items.find(i=>i.ItemID===l.ItemID)||{};
      const project=demo.projects.find(p=>p.ProjectID===l.ProjectID)||{};
      const team=demo.teams.find(t=>t.TeamID===item.AssignedTeamID)||{};
      return {...l,ClientName:project.ClientName||"",ProjectSummary:project.ProjectSummary||"",TeamID:item.AssignedTeamID||"",TeamName:team.TeamName||"",StartedAt:l.StartedAt||l.CreatedAt||"",FinishedAt:l.FinishedAt||""};
    });
    return ok({logs:JSON.parse(JSON.stringify(logs))});
  }
