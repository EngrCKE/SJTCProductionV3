  ...
  const logsHtml = logs.map(log=>`
    <div class="card">
      <div class="line" style="justify-content:space-between;align-items:flex-start">
        <div class="cardTitle">${escapeHtml(log.ToStatus)}</div>
        ${state.admin?`<button class="primary smallBtn" data-edit-log="${escapeAttr(log.LogID)}">Edit Log</button>`:""}
      </div>
      <div class="small">Personnel: ${escapeHtml(log.AssignedPersonnel||"—")}</div>
      <div class="small">Started: ${niceDT(log.StartedAt)}<br>Finished: ${niceDT(log.FinishedAt)}</div>
      <div class="small">Moved by: ${escapeHtml(log.MovedBy||"—")}</div>
      ${log.Remarks?`<div class="small">Remarks: ${escapeHtml(log.Remarks)}</div>`:""}
      ${log.CorrectionNote?`<div class="small">Correction: ${escapeHtml(log.CorrectionNote)}</div>`:""}
    </div>`).join("") || `<div class="hint">No logs yet.</div>`;
...
function openEditLogModal(logId){
  ...
  $("editLogModalBody").innerHTML = `
    ...
    <div class="twoCol">
      ...
      <div><label>Personnel Assigned</label><input id="editLogPersonnel" value="${escapeAttr(log.AssignedPersonnel || "")}" /></div>
