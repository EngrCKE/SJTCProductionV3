...
function applyMovementLogToLocal(log){
  if(!log || !log.ItemID) return;
  const item=state.items.find(i=>String(i.ItemID)===String(log.ItemID));
  if(!item) return;
  item.ItemStatus=log.ToStatus || item.ItemStatus;
  item.AssignedTeamID=typeof log.AssignedTeamID!=="undefined" ? log.AssignedTeamID : item.AssignedTeamID;
  item.AssignedPersonnel=typeof log.AssignedPersonnel!=="undefined" ? log.AssignedPersonnel : item.AssignedPersonnel;
  item.UpdatedAt=log.StartedAt || nowISO();
  item.CurrentLogID=log.LogID || item.CurrentLogID;
  delete state.loadedItemLogs[String(item.ItemID)];
}
function finishFastBoardMove(){
  state.pendingMove=null;
  state.productionLogsLoaded=false;
  closeModal("moveModal");
  setSync("Saved • full sync deferred");
  render();
}
...
async function confirmMove(){
  ...
  if(state.pendingMove.mode==="group"){
    ...
    (Array.isArray(result.results)?result.results:[]).forEach(applyMovementLogToLocal);
    finishFastBoardMove();
  ...
  const result=await api("moveProductionItem", { pin:accessPin(), itemId:state.pendingMove.itemId, toStatus:state.pendingMove.toStatus, assignedTeamId, assignedPersonnel, remarks:$("moveRemarks").value.trim(), movedBy:state.accessLevel==="admin"?"Admin":"Officer" });
  applyMovementLogToLocal(result && result.log);
  finishFastBoardMove();
}
