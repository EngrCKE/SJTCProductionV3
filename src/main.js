...
function bindGlobal(){
  document.querySelectorAll(".navBtn").forEach(t=>t.onclick=async()=>{ state.page=t.dataset.page; render(); if(state.page==="history"&&!state.historyLoaded){ try{ await ensureHistoryIndex(); }catch(err){ console.error(err); alert(err.message||err); } } });
