...
function bindGlobal(){
  ...
  $("btnAdminLogin").onclick=(e)=>runAction(e,"Unlocking access...", async()=>{ const pin=$("adminPinInput").value.trim(); const res=await api("validateAccess",{pin}); if(!res.valid) return alert("Invalid PIN"); localStorage.setItem(ADMIN_PIN_KEY,pin); closeModal("adminModal"); setAccess(res.role || "staff"); });
...
bindGlobal(); restoreBootstrapSnapshot(); load().then(startAutoRefresh);
